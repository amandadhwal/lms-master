...
<body>
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            ...
            if($result)
            {
                if (mysqli_num_rows($result) > 0)
                {
                    ...
                    {
                        $sql = "DELETE FROM `REGISTERED_USER` WHERE `Email` = '$email'";
                        $result = mysqli_query($conn, $sql);
                        if($result)
                        {
  ...
  background-position: 62% 38%">
            ...
            <h3 class="text-center" style="margin-top: 50px;">Delete Book Category</h3>
            <form action="/lms/delete_user.php" method="post" style="margin-left: 30px; margin-top: 50px;">
                <div class="mb-3">
                ...
                <button class="w-100 btn" style="background-color: #7CF5F9;" type="submit">Delete User</button>
            </form>
        </div>
        <div class="col-md-1"></div>
    </div>
